(function(){
  ...
  style.textContent=`.period-mini-spark{height:72px;min-width:250px;display:flex;align-items:flex-end;gap:6px;padding:5px 2px 20px}.period-mini-spark span{position:relative;flex:1;min-width:16px;border-radius:4px 4px 0 0;background:#66a1c9}.period-mini-spark span.negative{background:#da7279}.period-mini-spark small{position:absolute;left:50%;bottom:-17px;transform:translateX(-50%);font-size:8px;color:#8192a2}.period-mini-spark b{position:absolute;left:50%;top:-15px;transform:translateX(-50%);font-size:8px;color:#60788c;font-weight:600}.period-history-note{margin:0 0 16px;padding:10px 14px;border:1px solid #dce8f1;border-radius:10px;background:#f7fbfe;color:#5e778b;font-size:11px}`;
  document.head.appendChild(style);
  const spark=vals=>{const max=Math.max(...vals.map(v=>Math.abs(v)),1);return `<div class="period-mini-spark">${vals.map((v,i)=>`<span class="${v<0?'negative':''}" style="height:${Math.max(5,Math.abs(v)/max*46)}px"><b>${Math.abs(v)>=100?fmt(v,0):fmt(v,1)}</b><small>${months[i]}</small></span>`).join('')}</div>`};
  ...
  function renderJuly(){
    ...
    const page=document.querySelector('.page-content');if(page)page.insertAdjacentHTML('afterbegin','<div class="period-history-note">已保留 2026年6月数据；可使用顶部“历史数据期”下拉框切换查看。</div>');
    ...
    kpi.forEach((r,i)=>{if(k[i])k[i].innerHTML=`<span>${r[0]}</span><strong>${r[1]}</strong><div><b>${r[2]}</b><small>${r[3]}</small></div>`});
    ...
    setTable(0,['费用项目','1—7月趋势','2606','2607','本年累计','环比','变动说明'],op.map(r=>[r[0],spark(r[1]),fmt(r[1][5]),fmt(r[1][6]),fmt(r[2]),{v:`${mom(r[1][6],r[1][5])>=0?'+':''}${fmt(mom(r[1][6],r[1][5]))}%`,c:sign(mom(r[1][6],r[1][5]))},r[3]]));
    ...
    setTable(1,['重点费用','1—7月趋势','2606','2607','本年累计','环比'],focus.map(r=>[r[0],spark(r[1]),fmt(r[1][5]),fmt(r[1][6]),fmt(r[2]),{v:`${fmt(mom(r[1][6],r[1][5]))}%`,c:sign(mom(r[1][6],r[1][5]))}]));
    ...
    setTable(3,['项目','1—7月趋势','2607','源表汇总'],productivity.map(r=>[r[0],spark(r[1]),{v:fmt(r[1][6]),c:sign(r[1][6])},{v:fmt(r[2]),c:sign(r[2])}]));
    ...
    setTable(10,['项目','2606存货','2607存货','2607占比','1—7月周转率趋势','2607周转率'],inv.map(r=>[r[0],fmt(r[1]),fmt(r[2]),pct(r[3]),spark(r[4]),fmt(r[5])]));
